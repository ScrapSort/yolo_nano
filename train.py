...
if __name__ == "__main__":
	...
	for epoch in range(opt.epochs):
		...
		for batch_i, (_, imgs, targets,img_scores,gt_mix_index) in enumerate(dataloader):
			...
			metric_table = [["Metrics", *[f"YOLO Layer {i}" for i in range(3)]]]
			...
			for i, metric in enumerate(metrics):
				...
				row_metrics1 = formats[metric] % model.yolo_layer13.metrics.get(metric, 0)
				row_metrics2 = formats[metric] % model.yolo_layer26.metrics.get(metric, 0)
				row_metrics3 = formats[metric] % model.yolo_layer52.metrics.get(metric, 0)
				row_metrics = [row_metrics1, row_metrics2, row_metrics3]
				#row_metrics = [formats[metric] % yolo.metrics.get(metric, 0) for yolo in model.yolo_layers]
				metric_table += [[metric, *row_metrics]]

				# Tensorboard logging
				tensorboard_log = []
				metric_yolo_layers = [model.yolo_layer13, model.yolo_layer26, model.yolo_layer52]
				for j, yolo in enumerate(metric_yolo_layers):
